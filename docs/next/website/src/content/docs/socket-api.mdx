...
`pane.send_text`, `pane.send_keys`, `pane.send_input`, and `agent.send` are
composition-aware: sends queue per pane and flush only when no human
keystrokes reached the pane recently and the pane's visible composer holds no
human-typed text, so an API send never splices into a message a human is
typing. Queued sends flush in request order. Send responses report
`{"type":"pane_send","delivered":true|false,"queue_depth":N}`;
`pane.send_queued` / `pane.send_flushed` events track held sends, and
`pane.list` / `pane.get` expose `send_queue_depth` while sends are held (the
TUI shows a `✉N` badge on the pane border). Pass `"now":true` (CLI: a leading
`--now`) to bypass the queue and write immediately — reserve it for
interrupts such as `esc`. Panes whose agent detection manifest declares no
`[composer]` section gate on the keystroke debounce only.
...
`pane.get`, `pane.list`, `agent.get`, and `agent.list` expose a read-only `agent_session` object when Herdr has a stored native session reference. When the accepted integration report included a raw native session id or path, the pane and agent payloads also include `agent_session_id` and `agent_session_path`:

```json
{
  "agent_session_id": "claude-session",
  "agent_session_path": "/Users/me/.claude/projects/example/claude-session.jsonl",
  "agent_session": {
    "source": "herdr:claude",
    "agent": "claude",
    "kind": "id",
...
Use `pane.attach` when a protocol client needs live terminal bytes without
polling `pane.read`:

```json
{"id":"attach_1","method":"pane.attach","params":{"pane_id":"w1:p1"}}
```

The first line acknowledges the stream:

```json
{"id":"attach_1","result":{"type":"pane_attached","pane_id":"w1:p1"}}
```

Later lines are pushed byte chunks:

```json
{"id":"attach_1","stream":true,"chunk":{"pane_id":"w1:p1","seq":1,"encoding":"base64","data":"SGVsbG8K"}}
```

`data` is base64-encoded raw PTY output. `seq` is monotonic per attachment.
If the client falls far enough behind that chunks are dropped, Herdr closes the
stream so the client can reattach and recover from a fresh `pane.read`
snapshot.
