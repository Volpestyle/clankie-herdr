...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.attach`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Popup | `popup.close` |
...
`pane.output_changed`, `pane.output_matched`, `pane.agent_status_changed`, and `pane.scroll_changed`.
...
Raw `pane.read` responses include the pane output `revision`. Protocol clients
can pass `min_revision`; when the pane has not advanced beyond that revision,
Herdr returns the current `revision` with an empty `text` field instead of
walking and serializing terminal rows. Subscribe to `pane.output_changed` to
learn the next pane revision without polling.
...
Herdr keeps a bounded per-attachment message queue and byte budget. If the
client falls behind far enough to exhaust that budget, Herdr closes the stream
so the client can reattach and recover from a fresh `pane.read` snapshot.
